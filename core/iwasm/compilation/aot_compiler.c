...
bool
aot_emit_shared_file(AOTCompContext *comp_ctx, char *file_name)
{
    // the field belong to LLVN
    char *temp_name = "temp.a01924d3bb";
    LLVMCodeGenFileType file_type = LLVMSharedFile;
    // Call aot_emit_object_file to generate object file
    if(!aot_emit_object_file(comp_ctx,temp_name)){
        return false;
    }

    char cmd[1024];
    #if !(defined(_WIN32))
    snprintf(cmd,sizeof(cmd),"ld -shared -o %s %s",file_name, temp_name);
    if(system(cmd)!=0){
        return false;
    }
    return true;
